...
const Payslips = () => {
  ...
  useEffect(()=>{
    if(isAdmin) api.get("/employees").then((res)=>setEmployees(res.data.employees.filter((e)=>!e.isAdmin))).catch((error)=>toast.error(error.response?.data?.error || error.message));
  },[isAdmin]);
