      ...
      <div className="card overflow-hidden mb-4">
        <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Manage leave categories and configure their status.</p>
          </div>
          <button
            onClick={() => {
              setEditingType(null)
                ...
                leaveTypes.map((type) => (
                  <tr key={type._id || type.id}>
                    ...
                    <td className="text-center">
                      <button
                        onClick={() => onEdit(type)}
                        className="p-2.5 mx-1 bg-white-90 backdrop-blur-sm text-slate-700 hover:text-indigo-600 rounded-xl shadow-lg transition-all hover:scale-105 cursor-pointer"
                      >
                        <PencilIcon className="w-4 h-4 text-indigo-600" />
                      </button>
                      <button
                        onClick={() => handleDelete(type)}
                        className="p-2.5 mx-1 bg-white-90 backdrop-blur-sm text-slate-700 hover:text-indigo-600 rounded-xl shadow-lg transition-all hover:scale-105 disabled:opacity-50 cursor-pointer"
                      >
