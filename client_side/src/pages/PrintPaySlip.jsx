...
const PrintPaySlip = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white animate-fade-in p-8">
      <div className="text-center border-b borer-slate-200 pb-6 mb-8">
        <h1 className="text-2xl text-slate-900 font-bole tracking-tight">PAYSLIP</h1>
        <p className="text-slate-500 text-sm mt-1">{format(new Date(payslip.year,payslip.month-1),"MMM yyyy")}</p>
      </div>
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div>
          <p className="text-xl text-slate-400 uppercase tracking-wider mb-1">Employee Name</p>
          <p className="font-semibold text-slate-900">{payslip.employee?.firstName} {payslip.employee?.lastName}</p>
        </div>
        <div>
          <p className="text-xl text-slate-400 uppercase tracking-wider mb-1">Position</p>
          <p className="font-semibold text-slate-900">{payslip.employee?.position}</p>
        </div>
        <div>
          <p className="text-xl text-slate-400 uppercase tracking-wider mb-1">Email</p>
          <p className="font-semibold text-slate-900">{payslip.employee?.email}</p>
        </div>
        <div>
          <p className="text-xl text-slate-400 uppercase tracking-wider mb-1">Period</p>
